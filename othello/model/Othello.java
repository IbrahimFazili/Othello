package ca.utoronto.utm.othello.model;

import java.util.ArrayList;
import java.util.Random;

/**
 * Capture an Othello game. This includes an OthelloBoard as well as knowledge
 * of how many moves have been made, whosTurn is next (OthelloBoard.P1 or
 * OthelloBoard.P2). It knows how to make a move using the board and can tell
 * you statistics about the game, such as how many tokens P1 has and how many
 * tokens P2 has. It knows who the winner of the game is, and when the game is
 * over.
 * 
 * See the following for a short, simple introduction.
 * https://www.youtube.com/watch?v=Ol3Id7xYsY4
 * 
 * @author arnold
 *
 */
public class Othello {
	public static final int DIMENSION=8; // This is an 8x8 game

	private OthelloBoard board;
	private char whosTurn = OthelloBoard.P1;
	private int numMoves = 0;
	public boardChangeNotifier updateNotifier = new boardChangeNotifier(this);
	// stack to keep track of each stage
	private GameStateStack stateStack;

	public Othello() {
		this.stateStack = new GameStateStack();
		this.board = new OthelloBoard(Othello.DIMENSION);
	}

	/**
	 * restart the game
	 * */
	public void restart() {
		this.stateStack.clearStack();
		this.applyState(new GameStateCapture(new Othello()));
	}

	/**
	 * Force a GUI update
	 * */
	public void forceUpdate() { this.updateNotifier.updateAndNotify(this.board); }

	/**
	 * Apply the GameCaptureState to this game
	 *
	 * @param state the state that will be applied to the game
	 * */
	private void applyState(GameStateCapture state) {
		this.board = state.getBoard();
		this.numMoves = state.getNumMoves();
		this.whosTurn = state.getWhosTurn();
		this.updateNotifier.disableHint();
		this.forceUpdate();
	}

	/**
	 * Save the game state to the stateStack
	 * */
	public void captureState() { this.stateStack.push(new GameStateCapture(this)); }

	/**
	 * revert the game to the state it was in before the last
	 * user interaction
	 * */
	public void undo() {
		GameStateCapture lastState = this.stateStack.pop();
		if(lastState == null){ return; } // init state

		this.applyState(lastState);
	}

	public void deleteLastState() { this.stateStack.pop(); }

	public GameStateCapture peekLastState() { return this.stateStack.peek(); }

	/**
	 * return P1,P2 or EMPTY depending on who moves next.
	 * 
	 * @return P1, P2 or EMPTY
	 */
	public char getWhosTurn() { return this.whosTurn; }

	public void setGreedyHint(Hint greedyHint) {
		this.updateNotifier.setHint(greedyHint);
		this.updateNotifier.enableHint();
	}

	public void setRandomHint(Hint randomHint) {
		this.updateNotifier.setHint(randomHint);
		this.updateNotifier.enableHint();
	}

	/**
	 * @return the number of moves made so far
	 * */
	public int getNumMoves() { return this.numMoves; }

	public OthelloBoard getBoard() { return this.board; }

	/**
	 * 
	 * @param row 
	 * @param col
	 * @return the token at position row, col.
	 */
	public char getToken(int row, int col) {
		return this.board.get(row, col);
	}

	/**
	 * Generate an arraylist of possible moves for current player
	 *
	 * @return ArrayList of Move containing all valid moves
	 * */
	public ArrayList<Move> possibleMoves(){
		ArrayList<Move> theMoves = new ArrayList<Move>();
		for (int x = 0; x < 8; x++){
			for (int y =0; y < 8; y++){
				if (this.board.checkAllDirections(x, y, this.whosTurn)){
					theMoves.add(new Move(x, y));
				}
			}
		}
		return theMoves;
	}

	/**
	 * Attempt to make a move for P1 or P2 (depending on whos turn it is) at
	 * position row, col. A side effect of this method is modification of whos turn
	 * and the move count.
	 * 
	 * @param row
	 * @param col
	 * @return whether the move was successfully made.
	 */
	public boolean move(int row, int col) {
		if(this.board.move(row, col, this.whosTurn)) {
			this.whosTurn = OthelloBoard.otherPlayer(this.whosTurn);
			char allowedMove = board.hasMove();
			if(allowedMove != OthelloBoard.BOTH)this.whosTurn=allowedMove;
			this.numMoves++;
			if(this.updateNotifier.isHintRequired()) { this.updateNotifier.disableHint(); }
			this.updateNotifier.updateAndNotify(this.board);
			return true;
		} else {
			return false;
		}
	}

	/**
	 * 
	 * @param player P1 or P2
	 * @return the number of tokens for player on the board
	 */
	public int getCount(char player) {
		return board.getCount(player);
	}


	/**
	 * Returns the winner of the game.
	 * 
	 * @return P1, P2 or EMPTY for no winner, or the game is not finished.
	 */
	public char getWinner() {
		if(!this.isGameOver())return OthelloBoard.EMPTY;
		if(this.getCount(OthelloBoard.P1)> this.getCount(OthelloBoard.P2))return OthelloBoard.P1;
		if(this.getCount(OthelloBoard.P1)< this.getCount(OthelloBoard.P2))return OthelloBoard.P2;
		return OthelloBoard.EMPTY;
	}


	/**
	 * 
	 * @return whether the game is over (no player can move next)
	 */
	public boolean isGameOver() {
		return this.whosTurn==OthelloBoard.EMPTY;
	}

	/**
	 * 
	 * @return a copy of this. The copy can be manipulated without impacting this.
	 */
	public Othello copy() {
		Othello o= new Othello();
		o.board=this.board.copy();
		o.numMoves = this.numMoves;
		o.whosTurn = this.whosTurn;
		return o;
	}


	/**
	 * 
	 * @return a string representation of the board.
	 */
	public String getBoardString() {
		return board.toString()+"\n";
	}


	/**
	 * run this to test the current class. We play a completely random game. DO NOT
	 * MODIFY THIS!! See the assignment page for sample outputs from this.
	 * 
	 * @param args
	 */
	public static void main(String [] args) {
		Random rand = new Random();


		Othello o = new Othello();
		System.out.println(o.getBoardString());
		while(!o.isGameOver()) {
			int row = rand.nextInt(8);
			int col = rand.nextInt(8);

			if(o.move(row, col)) {
				System.out.println("makes move ("+row+","+col+")");
				System.out.println(o.getBoardString()+ o.getWhosTurn()+" moves next");
			}
		}

	}
}


